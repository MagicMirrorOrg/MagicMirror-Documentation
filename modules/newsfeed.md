...
| `allowedBasicHtmlTags` | Render a strict allowlist of basic inline HTML formatting tags in feed titles and descriptions instead of escaping them. Useful for feeds that encode emphasis as HTML (e.g. `<em>`). Any tag outside the safe list is ignored. All other HTML is escaped, so arbitrary HTML/script cannot be injected. <br><br> **Possible values:** any subset of `['b', 'strong', 'i', 'em', 'u', 'br', 'code', 's', 'sub', 'sup']` <br> **Default value:** `[]` (all HTML escaped) |
