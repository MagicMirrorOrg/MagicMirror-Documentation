...
| `showPrecipitationAmount`      | Show the amount of rain/snow.<br><br>Not supported by all providers, and some providers do not support all `type`s. Supported providers are `buienradar`, `envcanada`, `openmeteo`, `openweathermap`, `smhi`, `weatherbit` and `yr`.<br><br> **Possible values:** `true` or `false` <br> **Default value:** `false`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| `showPrecipitationProbability` | Show the probability of rain/snow.<br><br>Not supported by all providers, and some providers do not support all `type`s. Supported providers are `buienradar`, `envcanada`, `openmeteo`, `openweathermap`, `ukmetofficedatahub`, `weatherbit`, `weatherflow`, `weathergov` and `yr`.<br><br> **Possible values:** `true` or `false` <br> **Default value:** `false`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
### Buienradar (`buienradar`) options

**Note** that `buienradar` covers Netherlands and Belgium only, uses the metric
system, and does not require an API key.

::: warning TERMS OF USE

Buienradar is operated by DPG Media. Their
[terms of use](https://privacy.dpgmedia.nl/nl/document/terms-of-use) prohibit
commercial use. Make sure your use complies with the current terms before
deploying this provider.

:::

The `locationId` for a city can be looked up via the Buienradar location search,
e.g. `https://api.buienradar.nl/data/search/1.1/?query=Amsterdam` returns the ID
in the `places[].id` field.

| Option           | Description                                                                                                                                        |
| ---------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| `apiBase`        | The Buienradar forecast base URL.<br><br> **Default value:** `'https://forecast.buienradar.nl/2.0/forecast'`                                       |
| `locationId`     | The Buienradar location ID for the desired city. <br><br> **Possible value:** `6275` (Rotterdam) <br> This value is **REQUIRED**                   |
| `maxEntries`     | Maximum number of forecast entries to return for `forecast`/`daily`/`hourly`. <br><br> **Possible values:** `1` - `48` <br> **Default value:** `5` |
| `updateInterval` | How often the weather data is fetched in milliseconds. <br><br> **Default value:** `600000` (10 minutes)                                           |
